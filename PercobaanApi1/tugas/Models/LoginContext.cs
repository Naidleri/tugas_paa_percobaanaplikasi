...
namespace PercobaanApi1.Models
{
    public class LoginContext
    {
        ...
        public List<Login> Authentifikasi(string p_username, string p_password, IConfiguration p_config)
        {
            ...
            string query = string.Format(@"SELECT ps.id_person, ps.nama, ps.alamat, ps.email, pp.peran_fk, p.nama_peran
                FROM users.person ps
                INNER JOIN users.peran_person pp ON ps.id_person=pp.person_fk
                INNER JOIN users.peran p ON pp.peran_fk=p.id_peran
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    list1.Add(new Login()
                    {
                        ...
                        peran_fk = int.Parse(reader["peran_fk"].ToString()),
